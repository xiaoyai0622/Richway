...
function propanel_siteoptions_add_admin() {
    ...
    $tt_page = add_menu_page('metrodir CP', 'Metrodir CP', 'edit_theme_options', 'siteoptions','propanel_siteoptions_options_page', get_template_directory_uri().'/skl/framework/img/skl.png', 32);
    $tt_subpage = add_submenu_page( 'siteoptions', 'UOU Addons', 'UOU Addons', 'manage_options', 'siteoptions-plugins', 'skl_admin_options_plugin' );
    $tt_subpage_2 = add_submenu_page( 'siteoptions', 'UOU Update', 'UOU Update', 'manage_options', 'siteoptions-update', 'uou_admin_options_update' );

    add_action("admin_print_scripts-$tt_subpage_2", 'propanel_of_load_only');
    add_action("admin_print_styles-$tt_subpage_2",'propanel_of_style_only');
    add_action("admin_print_scripts-$tt_page", 'propanel_of_load_only');
	add_action("admin_print_styles-$tt_page",'propanel_of_style_only');
    add_action("admin_print_scripts-$tt_subpage", 'propanel_of_load_only');
    add_action("admin_print_styles-$tt_subpage",'propanel_of_style_only');
...
<div class="wrap" id="truethemes_container">
  ...
  <form action="" enctype="multipart/form-data" id="ofform">
    ...
    <input type="submit" name="update" value="Save All Changes" class="button-primary" />
  </form>
...
function propanel_of_style_only() {
    ...
    wp_enqueue_style('uoustyle', get_template_directory_uri().'/skl/plugin/admin-options/css/main.css');
...
function propanel_of_load_only() {
    ...
    wp_enqueue_script('gps', get_template_directory_uri().'/admin/js/gps.js', array('jquery'));
    wp_enqueue_script('uou', get_template_directory_uri().'/skl/plugin/admin-options/js/uou.js', array('jquery'));
...
<script type="text/javascript">
			jQuery(document).ready(function(){
                ...
                jQuery('#opt_metrodir_advsearch_1').addClass( 'hide');
  ...
  jQuery("a[title=submit]").click( function(){
    ...
    jQuery(this).parents("form").submit();
			...
			jQuery('#ofform').submit(function(){
					...
					var ajax_url = '<?php echo admin_url("admin-ajax.php"); ?>';
                        var $acc1a = jQuery('#opt_metrodir_acc_1_active');
                        var $acc1n = jQuery('#opt_metrodir_acc_1_name');
                        var $acc2a = jQuery('#opt_metrodir_acc_2_active');
                        var $acc2n = jQuery('#opt_metrodir_acc_2_name');
                        var $acc3a = jQuery('#opt_metrodir_acc_3_active');
                        var $acc3n = jQuery('#opt_metrodir_acc_3_name');
					...
					var data = {
						...
						<?php } ?>
						action: 'of_ajax_post_action',
                        act1active: $acc1a.val(),
                        act1name: $acc1n.val(),
                        act2active: $acc2a.val(),
                        act2name: $acc2n.val(),
                        act3active: $acc3a.val(),
                        act3name: $acc3n.val(),
	...
	foreach ($options as $value) {
		...
		//Start Heading
		 if ( $value['type'] != "heading" )
		 {
			...
			$output .= '<div class="section section-'.$value['type'].' '. $class .'">'."\n";
             if (isset( $value['type']) && $value['type'] == "cross_line" )
             {
                 $output .= '<div id="crossline"><h1>'.$value['title'].'</h1></div>'."\n";

                 $output .= '<div class="option"><div class="controls">';

             } else {
                 if (isset( $value['del_line']) && $value['del_line'] == true ) {  $output .= ''."\n";} else { $output .= '<h3 class="heading">'. $value['name'] .'</h3>'."\n";}



                 if(isset($value['social_icons'])){
                     $output .= '<i class="fa fa-'.$value['social_icons'].' fa-2x"></i>'."\n";
                 }
		...
		break;
            ...
            case 'gps_convert':
                ...
                $output .= '<input id="metrodir_convert_gps" type="button" value="Convert address to GPS coordinates" onclick="gps_convert();" />';
                $output .= '<label id="metrodir_convert_gps_log"></label>';
                break;


            case 'gps_convert_cntr':

                $output .= '<input id="metrodir_cntr_convert_gps" type="button" value="Convert address to GPS coordinates" onclick="gps_convert_cntr();" />';
                $output .= '<label id="metrodir_cntr_convert_gps_log"></label>';
                break;


            case 'advsearch_input':

                $output .= '<input id="metrodir_cntr_convert_gps" type="button" value="Add additional search field" onclick="gps_convert_cntr();" />';
                $output .= '<label id="metrodir_cntr_convert_gps_log"></label>';
                break;
