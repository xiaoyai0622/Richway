...
jQuery(function() {
    ...
    var $loadingwheel = jQuery('<img style="padding: 0 20px;" src="'+$homeurl+'/images/loader.gif" width="15px" height="15px">');
    var $noticeMsg = jQuery('<div>', {id: 'notice-msg'}).css('display', 'none');

    $form.parent().before($noticeMsg); // render invisible div for message | template in $noticeMsg |

    $button.click(function(){
        $button.after($loadingwheel);
        jQuery.post(ajaxUrl, $form.serialize(), function(data){ // post data to options page
            $noticeMsg.html(data).fadeIn(400);
            $loadingwheel.remove();
            setTimeout(function(){
                $noticeMsg.fadeOut(600, function(){
                    $noticeMsg.empty();
                });
            }, 4000);
        });
...
// Admin control collapsed settings
jQuery(document).ready(function($){
    //Meta Box Options
    var open_close = $('<a href="#" style="display:block; float:right; clear:right; margin:10px;">aa</a>');
