...
function loader() {
    ...
    var marker;
    var zoom;
    var start_lat = jQuery("#metro_address_lat").val();
    var start_lng = jQuery("#metro_address_lng").val();
    ...
    google.maps.event.addListener(marker, 'drag', function() {
        $("#metro_address_lat").val(marker.getPosition().lat());
        $("#metro_address_lng").val(marker.getPosition().lng());
    });
        ...
        geocoder.geocode({latLng: marker.getPosition(), region: 'en' }, function(results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
                if (results) {
                    ...
                    $("#metro_address_country_name").val(" ");
                    $("#metro_address_region_name").val(" ");
                    $("#metro_address_name").val(" ");
                    $("#metro_address_zip").val(" ");
                    for (var index_res=0; index_res <= (results.length - 1); index_res++) {
                        ...
                        for (var index_count=0; index_count <= (results[index_res].address_components.length - 1); index_count++) {
                            ...
                            if ((results[index_res].address_components[index_count].types[0] == "country") && (marker_country == false)) {
                                $("#metro_address_country_name").val(results[index_res].address_components[index_count].long_name);
                                marker_country = true;
                            }
                            if ((results[index_res].address_components[index_count].types[0] == "administrative_area_level_1") && (marker_region == false)) {
                                $("#metro_address_region_name").val(results[index_res].address_components[index_count].long_name);
                                marker_region = true;
                            }
                            ...
                            if ((results[index_res].address_components[index_count].types[0] == "postal_code") && (marker_zip == false)) {
                                $("#metro_address_zip").val(results[index_res].address_components[index_count].long_name);
                                marker_zip = true;
                            }
                        }
                    }
                    for (index_count=0; index_count <= (results[0].address_components.length - 1); index_count++) {
                        if ((results[0].address_components[index_count].types[0] != "country") &&
                            ...
                            if ($("#metro_address_name").val() == " ") { $("#metro_address_name").val(results[0].address_components[index_count].long_name); }
                            else { $("#metro_address_name").val($("#metro_address_name").val() + ", " + results[0].address_components[index_count].long_name); }
                        }
                    }
                    if ((marker_country == true) || (marker_region == true) || (marker_address == true) || (marker_zip == true)) {
                        $("#metrodir_convert_gps_log").html('<span style="color: green;">Geocode was successful. Status: ' + status + '</span>');
                    }
                }
            } else {
                $("#metrodir_convert_gps_log").html('<span style="color: red;">Error! Geocode was not successful for the following reason: ' + status + '</span>');
            }
        });
    });
}
...
function initialize() {
    ...
    var start_lat = jQuery("#metro_address_lat").val();
    var start_lng = jQuery("#metro_address_lng").val();
    var country = $("#metro_address_country_name").val();
    var region = $("#metro_address_region_name").val();
    var address = $("#metro_address_name").val();
    var zip = $("#metro_address_zip").val();
    ...
    geocoder.geocode( { address: full_address, region: 'en'}, function(results, status) {
        if (status == google.maps.GeocoderStatus.OK) {
            ...
            $('#metro_address_lat').val(results[0].geometry.location.lat());
            $('#metro_address_lng').val(results[0].geometry.location.lng());
            $("#metrodir_convert_gps_log").html("Geocode was successful. Status: " + status);
